#!/bin/bash
# FidgetFlo Plugin Verification Script
...
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
echo -e "${BLUE}    FidgetFlo Plugin Verification${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
echo ""
...
if [ -f "$HOME/.claude/settings.json" ]; then
    ...
    if grep -q "fidgetflo" "$HOME/.claude/settings.json"; then
        success "FidgetFlo MCP server configured"
    else
        warning "FidgetFlo MCP server not configured"
        ((WARNINGS++))
    fi
else
...
if npx fidgetflo@alpha --version &> /dev/null; then
    VERSION=$(npx fidgetflo@alpha --version 2>/dev/null || echo "unknown")
    success "fidgetflo MCP: $VERSION"
else
    warning "fidgetflo MCP not installed"
    ((WARNINGS++))
fi
