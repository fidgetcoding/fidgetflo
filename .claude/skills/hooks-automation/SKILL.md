...
**Required:**
- FidgetFlo CLI installed (`npm install -g fidgetflo@alpha`)
- Claude Code with hooks enabled
- `.claude/settings.json` with hook configurations

**Optional:**
- MCP servers configured (fidgetflo, ruv-swarm, flow-nexus)
...
npx fidgetflo hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx fidgetflo hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx fidgetflo hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx fidgetflo hook pre-edit [options]
...
Examples:
  npx fidgetflo hook pre-edit --file "src/auth/login.js"
  npx fidgetflo hook pre-edit -f "config/db.js" --validate-syntax
  npx fidgetflo hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx fidgetflo hook pre-bash --command <cmd>
...
Examples:
  npx fidgetflo hook pre-bash -c "rm -rf /tmp/cache"
  npx fidgetflo hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx fidgetflo hook pre-task [options]
...
Examples:
  npx fidgetflo hook pre-task --description "Implement user authentication"
  npx fidgetflo hook pre-task -d "Continue API dev" --load-memory
  npx fidgetflo hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx fidgetflo hook pre-search --query <query>
...
Examples:
  npx fidgetflo hook pre-search -q "authentication middleware"
```
...
```bash
npx fidgetflo hook post-edit [options]
...
Examples:
  npx fidgetflo hook post-edit --file "src/components/Button.jsx"
  npx fidgetflo hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx fidgetflo hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx fidgetflo hook post-bash --command <cmd>
...
Examples:
  npx fidgetflo hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx fidgetflo hook post-task [options]
...
Examples:
  npx fidgetflo hook post-task --task-id "auth-implementation"
  npx fidgetflo hook post-task -t "api-refactor" --analyze-performance
  npx fidgetflo hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx fidgetflo hook post-search --query <query> --results <path>
...
Examples:
  npx fidgetflo hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx fidgetflo hook mcp-initialized --swarm-id <id>
...
```bash
npx fidgetflo hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx fidgetflo hook task-orchestrated --task-id <id>
...
```bash
npx fidgetflo hook neural-trained --pattern <name>
...
```bash
npx fidgetflo hook memory-sync --namespace <ns>
...
```bash
npx fidgetflo hook session-start --session-id <id>
...
```bash
npx fidgetflo hook session-restore --session-id <id>
...
Examples:
  npx fidgetflo hook session-restore --session-id "swarm-20241019"
  npx fidgetflo hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx fidgetflo hook session-end [options]
...
Examples:
  npx fidgetflo hook session-end --session-id "dev-session-2024"
  npx fidgetflo hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx fidgetflo hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx fidgetflo hook notify --message <msg>
...
Examples:
  npx fidgetflo hook notify -m "Task completed" --level info
  npx fidgetflo hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx fidgetflo hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx fidgetflo hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx fidgetflo hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx fidgetflo hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx fidgetflo hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx fidgetflo hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx fidgetflo hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx fidgetflo hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__fidgetflo__memory_persist {
  sessionId: "dev-2024"
}
...
for FILE in $FILES; do
  ...
  npx fidgetflo hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx fidgetflo hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx fidgetflo hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx fidgetflo metrics score --format json | jq -r '.truth_score')
...
npx fidgetflo hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx fidgetflo hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx fidgetflo hook session-restore \
...
npx fidgetflo hook session-start --session-id "fullstack-feature"
...
# Backend work
npx fidgetflo hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx fidgetflo hook post-edit \
  --file "api/profile.js" \
...
npx fidgetflo hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx fidgetflo hook post-edit \
...
npx fidgetflo hook session-start --session-id "debug-memory-leak"
...
npx fidgetflo hook pre-search --query "EventEmitter"
# ... search executes ...
npx fidgetflo hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx fidgetflo hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx fidgetflo hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx fidgetflo hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx fidgetflo hook post-edit \
    --file "$file" \
...
npx fidgetflo hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .fidgetflo/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx fidgetflo hook validate-config
```
...
- Ensure fidgetflo CLI is in PATH
...
- `npx fidgetflo init --hooks` - Initialize hooks system
- `npx fidgetflo hook --list` - List available hooks
- `npx fidgetflo hook --test <hook>` - Test specific hook
- `npx fidgetflo memory usage` - Manage memory
- `npx fidgetflo agent spawn` - Spawn agents
- `npx fidgetflo swarm init` - Initialize swarm
