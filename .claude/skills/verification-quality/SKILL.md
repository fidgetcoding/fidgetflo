...
- FidgetFlo installed (`npx fidgetflo@alpha`)
...
npx fidgetflo@alpha truth

# Run verification check
npx fidgetflo@alpha verify check

# Verify specific file with custom threshold
npx fidgetflo@alpha verify check --file src/app.js --threshold 0.98

# Rollback last failed verification
npx fidgetflo@alpha verify rollback --last-good
```
...
npx fidgetflo@alpha truth --period 7d

# View scores for specific agent
npx fidgetflo@alpha truth --agent coder --period 24h

# Find files/tasks below threshold
npx fidgetflo@alpha truth --threshold 0.8
```

**Output Formats:**
```bash
# Table format (default)
npx fidgetflo@alpha truth --format table

# JSON for programmatic access
npx fidgetflo@alpha truth --format json

# CSV for spreadsheet analysis
npx fidgetflo@alpha truth --format csv

# HTML report with visualizations
npx fidgetflo@alpha truth --format html --export report.html
```

**Real-time Monitoring:**
```bash
# Watch mode with live updates
npx fidgetflo@alpha truth --watch

# Export metrics automatically
npx fidgetflo@alpha truth --export .fidgetflo/metrics/truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx fidgetflo@alpha verify check --file src/app.js

# Verify directory recursively
npx fidgetflo@alpha verify check --directory src/

# Verify with auto-fix enabled
npx fidgetflo@alpha verify check --file src/utils.js --auto-fix
...
npx fidgetflo@alpha verify check --task task-123

# Verify with custom threshold
npx fidgetflo@alpha verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx fidgetflo@alpha verify check --task task-789 --verbose
```

**Batch Verification:**
```bash
# Verify multiple files in parallel
npx fidgetflo@alpha verify batch --files "*.js" --parallel

# Verify with pattern matching
npx fidgetflo@alpha verify batch --pattern "src/**/*.ts"

# Integration test suite
npx fidgetflo@alpha verify integration --test-suite full
```
...
npx fidgetflo@alpha verify check --json > verification.json
...
npx fidgetflo@alpha verify rollback --to-commit abc123

# Interactive rollback with preview
npx fidgetflo@alpha verify rollback --interactive
```

**Smart Rollback:**
```bash
# Rollback only failed files (preserve good changes)
npx fidgetflo@alpha verify rollback --selective

# Rollback with automatic backup
npx fidgetflo@alpha verify rollback --backup-first

# Dry-run mode (preview without executing)
npx fidgetflo@alpha verify rollback --dry-run
```
...
```bash
# JSON report
npx fidgetflo@alpha verify report --format json

# HTML report with charts
npx fidgetflo@alpha verify report --export metrics.html --format html

# CSV for data analysis
npx fidgetflo@alpha verify report --format csv --export metrics.csv

# Markdown summary
npx fidgetflo@alpha verify report --format markdown
```

**Time-based Reports:**
```bash
# Last 24 hours
npx fidgetflo@alpha verify report --period 24h

# Last 7 days
npx fidgetflo@alpha verify report --period 7d

# Last 30 days with trends
npx fidgetflo@alpha verify report --period 30d --include-trends

# Custom date range
npx fidgetflo@alpha verify report --from 2025-01-01 --to 2025-01-31
```
...
npx fidgetflo@alpha verify dashboard

# Custom port
npx fidgetflo@alpha verify dashboard --port 8080

# Export dashboard data
npx fidgetflo@alpha verify dashboard --export

# Dashboard with auto-refresh
npx fidgetflo@alpha verify dashboard --refresh 5s
```
...
Set verification preferences in `.fidgetflo/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".fidgetflo/metrics/truth-daily.json"
    }
  }
}
```
...
npx fidgetflo@alpha verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx fidgetflo@alpha verify check --threshold 0.90

# Set default threshold
npx fidgetflo@alpha config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx fidgetflo@alpha verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx fidgetflo@alpha swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx fidgetflo@alpha hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx fidgetflo@alpha train --verify --threshold 0.99
```
...
npx fidgetflo@alpha pair --verify --real-time

# Pair with custom threshold
npx fidgetflo@alpha pair --verify --threshold 0.97 --auto-fix
```
...
npx fidgetflo@alpha verify watch --directory src/

# Watch with auto-fix
npx fidgetflo@alpha verify watch --directory src/ --auto-fix

# Watch with notifications
npx fidgetflo@alpha verify watch --notify --threshold 0.95
```
...
npx fidgetflo@alpha truth --format json | \
  curl -X POST https://pushgateway.example.com/metrics/job/fidgetflo \
  -d @-
...
npx fidgetflo@alpha verify install-hook --pre-commit

# .git/hooks/pre-commit example:
#!/bin/bash
npx fidgetflo@alpha verify check --threshold 0.95 --json > /tmp/verify.json

score=$(jq '.overallScore' /tmp/verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx fidgetflo@alpha verify check --verbose' for details"
  exit 1
fi
...
npx fidgetflo@alpha truth --verbose --threshold 0.0

# Check specific criteria
npx fidgetflo@alpha verify check --verbose

# View agent-specific issues
npx fidgetflo@alpha truth --agent <agent-name> --format json
```
...
npx fidgetflo@alpha verify rollback --history
...
```bash
# Increase timeout
npx fidgetflo@alpha verify check --timeout 60s

# Verify in batches
npx fidgetflo@alpha verify batch --batch-size 10
```
...
- `npx fidgetflo@alpha pair` - Collaborative development with verification
- `npx fidgetflo@alpha train` - Training with verification feedback
- `npx fidgetflo@alpha swarm` - Multi-agent coordination with quality checks
- `npx fidgetflo@alpha report` - Generate comprehensive project reports
