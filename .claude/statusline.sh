...
V3_METRICS="${PROJECT_DIR}/.fidgetflo/metrics/v3-progress.json"
SECURITY_AUDIT="${PROJECT_DIR}/.fidgetflo/security/audit-status.json"
PERFORMANCE_METRICS="${PROJECT_DIR}/.fidgetflo/metrics/performance.json"
...
ACTIVE_PROCESSES=$(ps aux 2>/dev/null | grep -E "(agentic-flow|fidgetflo)" | grep -v grep | wc -l)

# Check for real-time activity data from swarm monitor
SWARM_ACTIVITY=".fidgetflo/metrics/swarm-activity.json"
...
CONTEXT_COLOR="${DIM}"
AUTOPILOT_STATUS=""
AUTOPILOT_STATE="${PROJECT_DIR}/.fidgetflo/data/autopilot-state.json"
...
INTEL_SCORE=0
INTEL_COLOR="${DIM}"
PATTERNS_DB="${PROJECT_DIR}/.fidgetflo/learning/patterns.db"
LEARNING_METRICS="${PROJECT_DIR}/.fidgetflo/metrics/learning.json"
ARCHIVE_DB="${PROJECT_DIR}/.fidgetflo/data/transcript-archive.db"
...
CURRENT_DIR=$(basename "$PROJECT_DIR" 2>/dev/null || echo "fidgetflo")

# Build colorful output with better formatting
OUTPUT=""

# Header Line: V3 Project + Branch + Integration Status
OUTPUT="${BOLD}${BRIGHT_PURPLE}▊ FidgetFlo V3 ${RESET}"
