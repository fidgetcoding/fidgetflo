---
name: sparc
description: Execute SPARC methodology workflows with FidgetFlo
---
...
```bash
# Run SPARC orchestrator (default)
npx fidgetflo sparc "build complete authentication system"

# Run a specific mode
npx fidgetflo sparc run architect "design API structure"
npx fidgetflo sparc run tdd "implement user service"

# Execute full TDD workflow
npx fidgetflo sparc tdd "implement user authentication"

# List all modes with details
npx fidgetflo sparc modes --verbose

# For alpha features
npx fidgetflo@alpha sparc run <mode> "your task"
```

### Option 3: Local Installation
```bash
# If fidgetflo is installed locally
./fidgetflo sparc "build complete authentication system"
./fidgetflo sparc run architect "design API structure"
```
...
```bash
# Store specifications
npx fidgetflo memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec

# Store architectural decisions
./fidgetflo memory store "arch_api" "RESTful microservices design" --namespace arch

# Query previous work
./fidgetflo memory query "authentication" --limit 10

# Export project memory
./fidgetflo memory export sparc-project-backup.json
```
...
./fidgetflo swarm "Build e-commerce platform" --strategy development --monitor --review

# Background optimization swarm
./fidgetflo swarm "Optimize system performance" --strategy optimization --background

# Distributed research swarm
./fidgetflo swarm "Analyze market trends" --strategy research --distributed --ui
```
...
```bash
./fidgetflo sparc run code "implement API" --non-interactive
./fidgetflo sparc tdd "user tests" --non-interactive --enable-permissions
```
...
See `/fidgetflo-help` for all available commands.
