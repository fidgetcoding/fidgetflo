...
- `mcp__fidgetflo__*` tools (preferred)
- `npx fidgetflo` commands (as fallback)
...
- Updated to: `Tool: mcp__fidgetflo__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__fidgetflo__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__fidgetflo__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__fidgetflo__*` format
...
The analysis directory is now fully compliant with the FidgetFlo command standards.