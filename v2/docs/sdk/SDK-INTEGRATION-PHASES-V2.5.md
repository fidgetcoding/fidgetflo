# FidgetFlo v2.5.0-alpha.130 - SDK Integration Phases
      ...
      Array.from({ length: count }, async (_, index) => {
        const stream = query({
          ...
          options: {
            ...
            mcpServers: {
              'fidgetflo-swarm': claudeFlowSwarmServer
            }
          }
        });
...
```typescript
// src/mcp/fidgetflo-swarm-server.ts
...
### Tasks
- [ ] Create `fidgetflo-swarm` in-process MCP server
...
*Updated phases for FidgetFlo v2.5.0-alpha.130 with critical and high priority features*