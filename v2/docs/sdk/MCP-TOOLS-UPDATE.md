...
**File**: `src/mcp/fidgetflo-tools.ts`
...
When the MCP server is running (`fidgetflo mcp start`), all 7 tools are available:

```typescript
// Example: Create checkpoint via MCP
mcp__fidgetflo__checkpoint_create({
  sessionId: "my-session",
  description: "Before deployment"
})

// Example: Fork session via MCP
mcp__fidgetflo__session_fork({
  sessionId: "base-session",
  forkOptions: {}
})

// Example: Pause query via MCP
mcp__fidgetflo__query_pause({
...
```typescript
// Via MCP tool
const result = await mcp__fidgetflo__checkpoint_create({
...
```typescript
// Via MCP tool
const fork = await mcp__fidgetflo__session_fork({
...
```typescript
// Via MCP tool
await mcp__fidgetflo__query_pause({
  sessionId: "long-running-analysis"
});

// Later, resume from exact point
await mcp__fidgetflo__query_resume({
  sessionId: "long-running-analysis"
});
```
...
**The fidgetflo MCP server now provides complete access to all SDK features through standardized MCP tool interfaces.**
