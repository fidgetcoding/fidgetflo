# Schema Update Summary - Full FidgetFlo Compatibility
...
All pre-trained ReasoningBank models have been updated to include **all required fidgetflo memory tables**. The models now have **12 tables** instead of 4-8 tables.
...
✅ **Fully compatible** with all fidgetflo commands
...
npx fidgetflo@alpha memory store api_key "sk-123" --namespace config
...
# Store a memory
npx fidgetflo@alpha memory store test "Hello World" --namespace demo

# Query it back
npx fidgetflo@alpha memory query "test" --namespace demo
...
# 3. Test it
npx fidgetflo@alpha memory query "test" --reasoningbank
```
...
**The models are now 100% compatible with all fidgetflo features!** 🎉
