...
```bash
npx fidgetflo auto agent [options]
```
...
```bash
npx fidgetflo auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx fidgetflo auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx fidgetflo auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx fidgetflo auto agent -t "Fix bug in login" -s minimal
```
