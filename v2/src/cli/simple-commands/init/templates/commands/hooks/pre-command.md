...
```bash
npx fidgetflo hook pre-command [options]
```
...
```bash
npx fidgetflo hook pre-command --command "npm install express"
```

### Safety validation

```bash
npx fidgetflo hook pre-command -c "rm -rf node_modules" --validate-safety
```

### Permission check

```bash
npx fidgetflo hook pre-command -c "sudo apt update" --check-permissions
```

### Dry run preview

```bash
npx fidgetflo hook pre-command -c "git push origin main" --dry-run
```
...
npx fidgetflo hook pre-command --command "your command here" --validate-safety
```
