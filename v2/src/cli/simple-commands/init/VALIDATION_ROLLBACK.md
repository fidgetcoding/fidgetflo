...
fidgetflo init --enhanced --sparc

# Enhanced with specific options
fidgetflo init --safe --sparc --force

# Validation only (no initialization)
fidgetflo init --validate-only

# Skip specific validations if needed
fidgetflo init --enhanced --skip-pre-validation --skip-backup
```
...
fidgetflo init --validate

# Skip specific validation phases
fidgetflo init --validate --skip-pre-init --skip-config --skip-mode-test

# Pre-initialization check only
fidgetflo init --validate --pre-init-only
```
...
fidgetflo init --rollback --full

# Partial rollback for specific phase
fidgetflo init --rollback --partial --phase sparc-init

# Interactive rollback (shows available points)
fidgetflo init --rollback

# List available backups and checkpoints
fidgetflo init --list-backups
```
...
- `FIDGETFLO_DEBUG`: Enable debug logging
- `FIDGETFLO_BACKUP_DIR`: Custom backup directory
- `FIDGETFLO_STATE_FILE`: Custom state file location
...
- **Storage**: Backups stored in `.fidgetflo-backups/`
...
4. **Partial Initialization**
   ...
   fidgetflo init --enhanced --sparc --force
   # Or start fresh
   fidgetflo init --rollback --full
   ```
...
2. **Clean State Reset**
   ...
   rm -rf memory/ coordination/ fidgetflo
   ```

3. **Fresh Installation**
   ```bash
   # Start completely fresh
   npx fidgetflo@latest init --sparc --force
   ```
