...
Learn how to use SPARC methodology for systematic TDD with FidgetFlo.
...
- FidgetFlo with SPARC modes configured
...
```bash
cd examples
../fidgetflo sparc run spec-pseudocode \
...
```bash
../fidgetflo sparc run integration \
...
```bash
../fidgetflo sparc run security-review \
...
../fidgetflo memory store auth_spec "$(cat ./output/auth-spec.md)"

# Retrieve for future work
../fidgetflo memory query auth_spec > ./restored-spec.md
```
...
### Authentication Flow
```bash
../fidgetflo sparc tdd "implement login flow" \
  --pattern "request-validate-authenticate-respond"
```

### CRUD Operations
```bash
../fidgetflo sparc tdd "implement user CRUD" \
  --pattern "rest-api" \
  --with-validation
```
...
```bash
../fidgetflo sparc tdd "implement event handlers" \
  --pattern "pub-sub" \
  --async
```
