...
```bash
# Phase 1: Setup
npx fidgetflo sparc run code "Setup Express server with health endpoint" --non-interactive

# Phase 2: CRUD with TDD
npx fidgetflo sparc run tdd "Implement CRUD operations for tasks" --non-interactive

# Phase 3: Validation
npx fidgetflo sparc run code "Add input validation and error handling" --non-interactive

# Phase 4: Documentation
npx fidgetflo sparc run docs-writer "Create REST API documentation" --non-interactive

# Phase 5: Security Review
npx fidgetflo sparc run security-review "Review API security" --non-interactive
```

### Option 2: Parallel Swarm Execution (~15 minutes)
```bash
# Complete implementation with parallel agents
npx fidgetflo swarm "Build complete simple REST API from architecture docs" \
