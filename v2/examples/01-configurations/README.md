...
This directory contains configuration files for FidgetFlo system settings and orchestration, organized by complexity and use case.
...
- **simple-config.json**: Getting started configuration with essential settings
  ```bash
  cd examples
  ../fidgetflo orchestrate --config ./01-configurations/basic/simple-config.json workflow.json
  ```

### Minimal Configurations (`minimal/`)
- **minimal-config.json**: Absolute minimum required - uses defaults for everything else
  ```bash
  ../fidgetflo sparc run tdd "create feature" --config ./01-configurations/minimal/minimal-config.json
  ```
...
- **production-config.json**: Enterprise-ready with Redis, monitoring, security, and load balancing
  ```bash
  ../fidgetflo orchestrate --config ./01-configurations/advanced/production-config.json workflow.json
  ```
...
- **testing-config.json**: Test generation and execution with coverage requirements
  ```bash
  ../fidgetflo test generate --config ./01-configurations/specialized/testing-config.json src/
  ```
...
```bash
../fidgetflo orchestrate --config ./01-configurations/development-config.json workflow.json
```
