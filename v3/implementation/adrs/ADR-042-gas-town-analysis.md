# ADR-042: Gas Town & Beads Analysis - Lessons for FidgetFlo V3
...
Steve Yegge released [Gas Town](https://github.com/steveyegge/gastown) on January 1, 2026, a multi-agent orchestration system built on top of [Beads](https://github.com/steveyegge/beads), his git-backed issue tracker. This ADR analyzes these systems and identifies lessons applicable to FidgetFlo V3.
...
| Role | Function | FidgetFlo Equivalent |
|------|----------|----------------------|
...
## Comparison: Gas Town vs FidgetFlo V3

### Similarities

| Feature | Gas Town | FidgetFlo V3 |
|---------|----------|----------------|
...
```bash
npx fidgetflo tmux attach --mayor
npx fidgetflo tmux crew cycle
```
...
```bash
npx fidgetflo formulas search "release"
npx fidgetflo formulas install @community/release-workflow
```
...
FidgetFlo uses AgentDB for vectors, but could add Git-backed audit logs for critical state.
...
| 8 | Building orchestrator | Gas Town, FidgetFlo |

**FidgetFlo V3 targets Stage 7-8 users.**
...
FidgetFlo V3 has advantages in:
- **Intelligence**: Vector search, neural learning
- **Ecosystem**: MCP, plugins, multi-model
- **Accessibility**: No tmux requirement

The ideal system would combine Gas Town's durability guarantees with FidgetFlo's intelligence features.
