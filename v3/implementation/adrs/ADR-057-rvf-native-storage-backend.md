...
`npx fidgetflo@latest` installs **1.3GB** across 914 packages with a 35-second cold start in Docker. The Docker optimization work (Dockerfile.lite with `--omit=optional` + aggressive pruning) reduced this to 324MB, but the **core dependency chain** still carries unnecessary weight:

```
fidgetflo (5KB wrapper)
  └─ @claude-flow/cli (9MB)
...
RVF (RuVector Format) is a binary container format already used in the FidgetFlo ecosystem (ADR-054). It provides everything sql.js does **plus native HNSW indexing** in a fraction of the footprint:
...
fidgetflo migrate status --storage

# Dry-run migration (report what would change, don't modify)
fidgetflo migrate run --storage --dry-run

# Migrate specific file
fidgetflo migrate run --storage --file ./data/memory/memory.db

# Migrate all storage files in project
fidgetflo migrate run --storage --all

# Force re-migration (even if .rvf already exists)
fidgetflo migrate run --storage --force

# Rollback: restore from .bak files
fidgetflo migrate rollback --storage

# Validate migrated data integrity
fidgetflo migrate validate --storage
```
...
4. **`fidgetflo migrate rollback --storage`** restores `.bak` → original and removes `.rvf`.
...
fidgetflo memory init --backend sqljs

# Force JSON backend
fidgetflo memory init --backend json

# Default (RVF)
fidgetflo memory init
```
...
| Task | Package | Description |
|------|---------|-------------|
| P4.1 | `@claude-flow/cli` | `fidgetflo migrate status --storage` — detect formats, report state |
| P4.2 | `@claude-flow/cli` | `fidgetflo migrate run --storage` — batch migration with progress |
| P4.3 | `@claude-flow/cli` | `fidgetflo migrate rollback --storage` — restore from `.bak` |
| P4.4 | `@claude-flow/cli` | `fidgetflo migrate validate --storage` — integrity verification |
...
| P7.3 | `@claude-flow/cli` | `fidgetflo capabilities status/install/remove/list/prefetch` CLI commands |
...
- **Loss of SQL tooling** — can't `sqlite3 memory.db` to inspect data (mitigated by `fidgetflo memory list`)
...
- `npx fidgetflo@latest` installs 1.3GB (all optional deps)
...
Phase 0: Core CLI (always installed)
  fidgetflo (5KB) → @claude-flow/cli (9MB) → @claude-flow/shared (~13MB with RVF)
...
Phase 1: Lightweight Embeddings (downloaded on first use)
  ...
  Downloaded to: ~/.fidgetflo/capabilities/rvf-wasm.rvf

Phase 2: Neural Embeddings (downloaded on demand)
  all-MiniLM-L6-v2 ONNX model (~22MB)
  Stored as: ~/.fidgetflo/capabilities/models/minilm-l6-v2.rvf
...
Phase 3: Local LLM Inference (downloaded on demand)
  ...
  Stored as: ~/.fidgetflo/capabilities/models/<model>.rvf
...
Phase 4: Advanced Intelligence (downloaded on demand)
  ...
  Stored as: ~/.fidgetflo/capabilities/kernels/<kernel>.rvf
...
export class ProgressiveDownloader {
  ...
  constructor(rufloHome = '~/.fidgetflo') {
  ...
  async ensure(capabilityId: string): Promise<string> {
    ...
    console.info(`[fidgetflo] Downloading ${entry.name} (${this.formatSize(entry.size)})...`);
    ...
    console.info(`[fidgetflo] ✓ ${entry.name} ready`);
    return rvfPath;
  }
...
fidgetflo capabilities status

# Download specific capability
fidgetflo capabilities install neural-embeddings

# Download all capabilities up to phase N
fidgetflo capabilities prefetch --phase 2

# Remove a capability
fidgetflo capabilities remove local-llm-qwen

# List all available models/kernels
fidgetflo capabilities list --phase 3
fidgetflo capabilities list --type model
fidgetflo capabilities list --type kernel
```
