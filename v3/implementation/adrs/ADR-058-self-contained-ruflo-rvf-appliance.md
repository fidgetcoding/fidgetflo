# ADR-058: Self-Contained FidgetFlo RVF Appliance — Linux Kernel + Claude Code + ruvLLM
...
FidgetFlo v3.5 requires the user to have Node.js 20+, npm, Claude Code CLI, API keys, and a properly configured OS environment. This means:
...
A **single `fidgetflo.rvf` file** that contains everything needed to run the full FidgetFlo platform:

```
fidgetflo.rvf (self-contained appliance)
├── Linux microkernel (Alpine-based, ~5MB)
├── Node.js 22 runtime (~30MB stripped)
├── Claude Code CLI
├── FidgetFlo v3.5+ (all packages)
...
Extending RVF to `RVFA` (RuVector Format Appliance) creates a unified format that FidgetFlo already understands natively.
...
│   /sbin/init → fidgetflo-init (PID 1)              │
├─────────────────────────────────────────────────┤
│ Section 1: RUNTIME (compressed)                 │
│   Node.js 22 (stripped, no npm)                 │
│   Claude Code CLI binary                        │
├─────────────────────────────────────────────────┤
│ Section 2: FIDGETFLO (compressed)                   │
...
|------|---------|-------------|
| **Run** | `fidgetflo-appliance run fidgetflo.rvf` | Boot and enter interactive CLI |
| **MCP** | `fidgetflo-appliance mcp fidgetflo.rvf` | Boot as MCP server (stdio) |
| **Verify** | `fidgetflo-appliance verify fidgetflo.rvf` | Run full capability test suite |
| **Extract** | `fidgetflo-appliance extract fidgetflo.rvf ./out/` | Unpack all sections |
| **Build** | `fidgetflo-appliance build --profile offline` | Create new appliance |
| **Update** | `fidgetflo-appliance update fidgetflo.rvf --section FIDGETFLO` | Hot-patch one section |
| **Inspect** | `fidgetflo-appliance inspect fidgetflo.rvf` | Show header + section manifest |
...
fidgetflo-appliance run fidgetflo.rvf
# Equivalent to:
# docker run --rm -it fidgetflo:self-contained
```
...
The appliance includes a built-in verification suite that tests **every capability** of FidgetFlo + FidgetFlo. This runs automatically at boot (`Section 5: VERIFY`) and can be triggered manually.
...
| 35 | Hot Update | patch FIDGETFLO section → verify new version | Live update |

### 3.3 Test Output Format

```
╔══════════════════════════════════════════════════════════╗
║  FidgetFlo Appliance v3.5.2 — Full Capability Verification  ║
...
fidgetflo-appliance build \
  --profile offline \
  --arch x86_64 \
  --models "phi-3-mini-q4,qwen2.5-coder-3b-q4" \
  --output fidgetflo-offline.rvf

# Build cloud appliance (API keys only)
fidgetflo-appliance build \
  --profile cloud \
  --api-keys .env \
  --output fidgetflo-cloud.rvf
...
Stage 3: FIDGETFLO
  ├── npm pack fidgetflo@latest --omit=optional
...
|---------|----------|-----------------|
| `cloud` | Kernel + Runtime + FidgetFlo + Data + Verify | ~60MB |
...
| `fidgetflo-appliance build` | Multi-stage builder with profile selection |
| `fidgetflo-appliance inspect` | Header + section manifest viewer |
| `fidgetflo-appliance extract` | Unpack all sections to directory |
| Cloud profile | Kernel + Runtime + FidgetFlo + encrypted keys |

### Phase 2: Runtime + Verification (Week 3-4)

| Task | Description |
|------|-------------|
| `fidgetflo-appliance run` | Boot sequence with container isolation |
| `fidgetflo-appliance verify` | 95-check capability suite |
| `fidgetflo-appliance mcp` | MCP server mode (stdio + SSE) |
...
| Task | Description |
|------|-------------|
| `fidgetflo-appliance update` | Hot-patch individual sections |
...
- **Fast boot**: <5s from cold start (vs 35s for `npx fidgetflo@latest`)
