...
| **Workflow persistence** | Saves to `.fidgetflo/workflows/store.json` | `workflow-tools.ts:52-78` — `loadWorkflowStore()`/`saveWorkflowStore()` |
...
| **P2** | Intelligence stats → neural bridge | Stats counter disconnected | Read from ReasoningBank `.fidgetflo/neural/patterns.json` |
...
- Read pattern count from `.fidgetflo/neural/patterns.json`
- Read trajectory count from `.fidgetflo/neural/stats.json`
