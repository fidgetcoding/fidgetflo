# FidgetFlo V3 Complete CLI Reference

## Overview

Complete command reference for the fidgetflo V3 CLI including hooks, workflow execution, hive-mind coordination, process management, and all subcommands with SONA/MoE/HNSW integration.
...
```bash
# Hooks are available through the main fidgetflo CLI
npm install -g @claude-flow/cli

# Or via npx
npx fidgetflo hooks --help
```
...
```bash
npx fidgetflo hooks pre-edit <filePath> [options]
...
Examples:
  npx fidgetflo hooks pre-edit src/auth.ts
  npx fidgetflo hooks pre-edit src/new-file.ts --operation create
  npx fidgetflo hooks pre-edit src/legacy.ts --no-suggestions --format json
```
...
```bash
npx fidgetflo hooks post-edit <filePath> [options]
...
Examples:
  npx fidgetflo hooks post-edit src/auth.ts --success true
  npx fidgetflo hooks post-edit src/auth.ts --success false --outcome "Type error on line 42"

  # V2 compatibility
  npx fidgetflo hooks post-edit --file src/auth.ts --success true --memory-key "swarm/coder/edit1"
```
...
```bash
npx fidgetflo hooks pre-command "<command>" [options]
...
Examples:
  ...
  npx fidgetflo hooks pre-command "rm -rf ./dist" --working-dir /project
  npx fidgetflo hooks pre-command "docker compose up" --format json
```
...
```bash
npx fidgetflo hooks post-command "<command>" [options]
...
Examples:
  npx fidgetflo hooks post-command "npm test" --success true --time 5230
  npx fidgetflo hooks post-command "npm build" --success false --exit-code 1 --error "Module not found"
```
...
```bash
npx fidgetflo hooks pre-task [options]
...
Examples:
  npx fidgetflo hooks pre-task --description "Implement OAuth2 flow"
  npx fidgetflo hooks pre-task -d "Fix login bug" --agent debugger --task-id task-123
```
...
```bash
npx fidgetflo hooks post-task [options]
...
Examples:
  npx fidgetflo hooks post-task --task-id task-123 --success true
  npx fidgetflo hooks post-task --task-id task-123 --success false --result "Blocked by dependency"
```
...
```bash
npx fidgetflo hooks session-restore [options]
...
Examples:
  npx fidgetflo hooks session-restore --session-id swarm-abc123
  npx fidgetflo hooks session-restore --session-id previous --include-memory false
```
...
```bash
npx fidgetflo hooks session-end [options]
...
Examples:
  ...
  npx fidgetflo hooks session-end --export-metrics true --summary
```
...
```bash
npx fidgetflo hooks route "<task>" [options]
...
Examples:
  ...
  npx fidgetflo hooks route "Fix CSS bug" --prefer "coder,reviewer"
  npx fidgetflo hooks route "Research API options" --context "REST vs GraphQL" --format json
```
...
```bash
npx fidgetflo hooks explain "<task>" [options]
...
Examples:
  npx fidgetflo hooks explain "Implement OAuth2 authentication"
  npx fidgetflo hooks explain "Security audit" --verbose
```
...
```bash
npx fidgetflo hooks pretrain [options]
...
Examples:
  ...
  npx fidgetflo hooks pretrain --path /project --max-patterns 5000
  npx fidgetflo hooks pretrain --force --no-include-git
```
...
```bash
npx fidgetflo hooks build-agents [options]
...
Examples:
  ...
  npx fidgetflo hooks build-agents --output agents.json
  npx fidgetflo hooks build-agents --v3-mode --focus performance
```
...
```bash
npx fidgetflo hooks transfer <sourceProject> [options]
...
Examples:
  npx fidgetflo hooks transfer /other-project
  npx fidgetflo hooks transfer ../shared-project --filter "security/*"
  npx fidgetflo hooks transfer /template --dry-run
```
...
```bash
npx fidgetflo hooks metrics [options]
...
Examples:
  ...
  npx fidgetflo hooks metrics --category routing --time-range week
  npx fidgetflo hooks metrics --detailed --format json
  npx fidgetflo hooks metrics --v3-dashboard
```
...
```bash
npx fidgetflo hooks list [options]
...
Examples:
  npx fidgetflo hooks list
  npx fidgetflo hooks list --category routing
  npx fidgetflo hooks list --include-disabled --format json
```
...
```bash
npx fidgetflo hooks notify [options]
...
Examples:
  npx fidgetflo hooks notify --message "Task completed successfully"
  npx fidgetflo hooks notify -m "Build failed" --level error
```
...
FIDGETFLO_HOOK_TIMEOUT=5000

# Enable/disable ReasoningBank integration
FIDGETFLO_REASONINGBANK_ENABLED=true

# Learning namespace
FIDGETFLO_HOOKS_NAMESPACE=hooks-learning

# Logging level
FIDGETFLO_HOOKS_LOG_LEVEL=info
```
...
npx fidgetflo hooks session-restore --session-id "swarm-[id]"
npx fidgetflo hooks post-edit --file "[file]" --memory-key "swarm/[agent]/[step]"
npx fidgetflo hooks notify --message "[what was done]"
npx fidgetflo hooks post-task --task-id "[task]"
...
```
$ npx fidgetflo hooks route "Implement authentication"
...
```bash
$ npx fidgetflo hooks route "Implement authentication" --format json
```
...
```
$ npx fidgetflo hooks metrics --format table
...
```bash
npx fidgetflo hooks intelligence [options]
```
...
```bash
npx fidgetflo workflow run <name|file> [options]
```
...
**Example:**
```bash
npx fidgetflo workflow run development --input '{"feature": "auth"}' --parallel 6
```
...
```bash
npx fidgetflo workflow validate <file> [options]
```
...
```bash
npx fidgetflo workflow list [options]
```
...
```bash
npx fidgetflo workflow status [options]
```
...
```bash
npx fidgetflo workflow stop [options]
```
...
```bash
# List templates
npx fidgetflo workflow template list

# Show template details
npx fidgetflo workflow template show <name>

# Create new template
npx fidgetflo workflow template create --name <name> --output <file>
```
...
**Example:**
```bash
npx fidgetflo hive-mind init --topology hierarchical-mesh --consensus byzantine --workers 15
```
...
```bash
npx fidgetflo hive-mind spawn [options]
```
...
```bash
npx fidgetflo hive-mind status [options]
```
...
```bash
npx fidgetflo hive-mind task [options]
```
...
```bash
npx fidgetflo hive-mind optimize-memory [options]
```
...
```bash
npx fidgetflo hive-mind shutdown [options]
```
...
```bash
npx fidgetflo agent list [options]
```
...
```bash
npx fidgetflo agent pool [options]
```
...
```bash
npx fidgetflo agent health [options]
```
...
```bash
npx fidgetflo agent logs [options]
```
...
```bash
npx fidgetflo memory store --key <key> --value <value> [options]
```
...
```bash
npx fidgetflo memory retrieve --key <key> [options]
```
...
```bash
npx fidgetflo memory search --query "<query>" [options]
```
...
```bash
npx fidgetflo memory stats [options]
```
...
```bash
npx fidgetflo memory cleanup [options]
```
...
```bash
npx fidgetflo memory compress [options]
```
...
```bash
# Export memory
npx fidgetflo memory export --output <file> [options]

# Import memory
npx fidgetflo memory import --input <file> [options]
```
...
```bash
npx fidgetflo process daemon --action <action> [options]
```
...
| `--pid-file` | string | `.fidgetflo/daemon.pid` | PID file location |
| `--log-file` | string | `.fidgetflo/daemon.log` | Log file location |
...
```bash
npx fidgetflo process monitor [options]
```
...
```bash
npx fidgetflo process workers --action <action> [options]
```
...
```bash
npx fidgetflo process signals --target <target> --signal <signal> [options]
```
...
```bash
npx fidgetflo process logs [options]
```
...
```bash
npx fidgetflo swarm status [options]
```
...
```bash
npx fidgetflo swarm task --description "<task>" [options]
```
...
|----------|-------------|---------|
| `FIDGETFLO_CONFIG` | Config file path | `.fidgetflo/config.json` |
| `FIDGETFLO_LOG_LEVEL` | Log level | `info` |
| `FIDGETFLO_MEMORY_BACKEND` | Memory backend | `agentdb` |
| `FIDGETFLO_EMBEDDING_PROVIDER` | Embedding provider | `transformers` |
| `FIDGETFLO_HOOK_TIMEOUT` | Hook timeout (ms) | `5000` |
| `FIDGETFLO_REASONINGBANK_ENABLED` | ReasoningBank integration | `true` |
