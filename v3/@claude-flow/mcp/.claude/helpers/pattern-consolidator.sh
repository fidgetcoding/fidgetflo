#!/bin/bash
# FidgetFlo V3 - Pattern Consolidator Worker
...
PATTERNS_DB="$PROJECT_ROOT/.fidgetflo/learning/patterns.db"
...
case "${1:-check}" in
  ...
  "status")
    if [ -f "$PATTERNS_DB" ] && command -v sqlite3 &>/dev/null; then
      # NOTE: no `local` here — this runs at case-statement level, not in a
      # function, and `local` outside a function is a fatal error under set -e
