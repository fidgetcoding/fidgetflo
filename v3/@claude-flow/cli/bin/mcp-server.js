  ...
  for (const line of lines) {
    if (line.trim()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}] ERROR [fidgetflo-mcp] Failed to parse:`,
...
process.on('SIGINT', () => {
  console.error(`[${new Date().toISOString()}] INFO [fidgetflo-mcp] Received SIGINT`);
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.error(`[${new Date().toISOString()}] INFO [fidgetflo-mcp] Received SIGTERM`);
  process.exit(0);
});
  ...
  try {
    switch (message.method) {
      case 'initialize':
        return {
          ...
          result: {
            ...
            serverInfo: { name: 'fidgetflo', version: VERSION },
            capabilities: {
      ...
      case 'notifications/initialized':
        console.error(`[${new Date().toISOString()}] INFO [fidgetflo-mcp] Client initialized`);
    ...
    console.error(`[${new Date().toISOString()}] ERROR [fidgetflo-mcp] ${message.method}:`, error);
    return {
      jsonrpc: '2.0',
      id: message.id,
      error: {
