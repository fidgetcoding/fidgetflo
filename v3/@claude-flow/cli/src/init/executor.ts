  ...
  try {
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use FidgetFlo'
      };
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use FidgetFlo'
      };
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add fidgetflo allow rules)
...
async function writeRuntimeConfig(
  ...
  const config = `# FidgetFlo Runtime Configuration
...
async function writeInitialMetrics(
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use FidgetFlo. Run: npx fidgetflo daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
...
/**
 * Write CAPABILITIES.md - comprehensive overview of all FidgetFlo features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilities = `# FidgetFlo - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
...
FidgetFlo is a domain-driven design architecture for multi-agent AI coordination with:
...
# Agents
npx fidgetflo agent spawn -t coder
npx fidgetflo agent list

# Memory
npx fidgetflo memory search --query "patterns"

# Hooks
npx fidgetflo hooks pre-task --description "task"
npx fidgetflo hooks worker dispatch --trigger optimize
\`\`\`
...
async function writeClaudeMd(
  ...
  // Also write/append global ~/.claude/CLAUDE.md so fidgetflo tools are used automatically (#1497)
  const homeDir = process.env.HOME || process.env.USERPROFILE || '';
  if (homeDir) {
    const globalClaudeDir = path.join(homeDir, '.claude');
    const globalClaudeMd = path.join(globalClaudeDir, 'CLAUDE.md');
    const fidgetfloBlock = [
      '',
      '# FidgetFlo Integration (auto-generated by fidgetflo init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke fidgetflo MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('FidgetFlo Integration')) {
          fs.appendFileSync(globalClaudeMd, fidgetfloBlock);
          result.created.files.push('~/.claude/CLAUDE.md (appended fidgetflo block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, fidgetfloBlock.trimStart(), 'utf-8');
