 ...
 * Session management for FidgetFlo
 */
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "fidgetflo session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: fidgetflo session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "fidgetflo start"');
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'fidgetflo session list', description: 'List all sessions' },
    { command: 'fidgetflo session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'fidgetflo session restore session-123', description: 'Restore a session' },
    { command: 'fidgetflo session delete session-123', description: 'Delete a session' },
    { command: 'fidgetflo session export -o backup.json', description: 'Export session to file' },
    { command: 'fidgetflo session import backup.json', description: 'Import session from file' },
    { command: 'fidgetflo session current', description: 'Show current session' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: fidgetflo session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "fidgetflo session <subcommand> --help" for subcommand help');
