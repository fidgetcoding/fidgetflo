...
function generateBashCompletion(): string {
  return `# fidgetflo bash completion
# Generated by fidgetflo completions bash
...
complete -F _claude_flow_completions fidgetflo
...
function generateZshCompletion(): string {
  return `#compdef fidgetflo
# fidgetflo zsh completion
# Generated by fidgetflo completions zsh
...
_claude_flow() {
    ...
    case $state in
        command)
            _describe -t commands 'fidgetflo commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# fidgetflo fish completion
# Generated by fidgetflo completions fish
# Save to ~/.config/fish/completions/fidgetflo.fish

# Disable file completion by default
complete -c fidgetflo -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c fidgetflo -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c fidgetflo -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c fidgetflo -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c fidgetflo -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c fidgetflo -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c fidgetflo -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c fidgetflo -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c fidgetflo -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# fidgetflo PowerShell completion
# Generated by fidgetflo completions powershell
...
Register-ArgumentCompleter -Native -CommandName fidgetflo -ScriptBlock {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'fidgetflo completions bash > ~/.bash_completion.d/fidgetflo', description: 'Install bash completions' },
    { command: 'fidgetflo completions zsh > ~/.zfunc/_claude-flow', description: 'Install zsh completions' },
    { command: 'fidgetflo completions fish > ~/.config/fish/completions/fidgetflo.fish', description: 'Install fish completions' },
    { command: 'fidgetflo completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  fidgetflo completions bash > ~/.bash_completion.d/fidgetflo'));
    output.writeln(output.dim('  source ~/.bash_completion.d/fidgetflo'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  fidgetflo completions zsh > ~/.zfunc/_claude-flow'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  fidgetflo completions fish > ~/.config/fish/completions/fidgetflo.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  fidgetflo completions powershell >> $PROFILE'));
