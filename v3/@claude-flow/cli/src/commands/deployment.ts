...
function getStateDir(cwd: string): string {
  return path.join(cwd, '.fidgetflo');
}
...
const deployCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment deploy -e prod', description: 'Deploy to production' },
    { command: 'fidgetflo deployment deploy --dry-run', description: 'Simulate deployment' },
  ],
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment status', description: 'Show all environments' },
    { command: 'fidgetflo deployment status -e prod', description: 'Check production' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const state = loadDeploymentState(ctx.cwd);
...
const rollbackCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment rollback -e prod', description: 'Rollback production' },
    { command: 'fidgetflo deployment rollback -e prod -v v3.0.0', description: 'Rollback to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
...
const historyCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment history', description: 'Show all history' },
    { command: 'fidgetflo deployment history -e prod', description: 'Production history' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const state = loadDeploymentState(ctx.cwd);
...
const environmentsCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment environments', description: 'List environments' },
    { command: 'fidgetflo deployment envs -a add -n preview -t staging', description: 'Add environment' },
    { command: 'fidgetflo deployment envs -a remove -n preview', description: 'Remove environment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment logs -e prod', description: 'View production logs' },
    { command: 'fidgetflo deployment logs -d dep-123', description: 'View specific deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const state = loadDeploymentState(ctx.cwd);
...
const releaseCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment release -v 3.5.0', description: 'Release version 3.5.0' },
    { command: 'fidgetflo deployment release -v 3.5.0 -d "Major update"', description: 'Release with description' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
...
export const deploymentCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo deployment deploy -e prod', description: 'Deploy to production' },
    { command: 'fidgetflo deployment status', description: 'Check all environments' },
    { command: 'fidgetflo deployment rollback -e prod', description: 'Rollback production' },
    { command: 'fidgetflo deployment release -v 3.5.0', description: 'Create a release' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('FidgetFlo Deployment'));
