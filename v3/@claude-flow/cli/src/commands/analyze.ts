...
const diffCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze diff --risk', description: 'Analyze current diff with risk assessment' },
    { command: 'fidgetflo analyze diff HEAD~1 --classify', description: 'Classify changes from last commit' },
    { command: 'fidgetflo analyze diff main..feature --format json', description: 'Compare branches with JSON output' },
    { command: 'fidgetflo analyze diff --reviewers', description: 'Get recommended reviewers for changes' },
  ],
...
const codeCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze code -p ./src', description: 'Analyze source directory' },
    { command: 'fidgetflo analyze code --type complexity', description: 'Run complexity analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const astCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze ast src/', description: 'Analyze all files in src/' },
    { command: 'fidgetflo analyze ast src/index.ts --complexity', description: 'Analyze with complexity' },
    { command: 'fidgetflo analyze ast . --format json', description: 'JSON output' },
    { command: 'fidgetflo analyze ast src/ --symbols', description: 'Extract symbols' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const complexityAstCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze complexity src/', description: 'Analyze complexity' },
    { command: 'fidgetflo analyze complexity src/ --threshold 15', description: 'Flag high complexity' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const symbolsCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze symbols src/', description: 'Extract all symbols' },
    { command: 'fidgetflo analyze symbols src/ --type function', description: 'Only functions' },
    { command: 'fidgetflo analyze symbols src/ --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const importsCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze imports src/', description: 'Analyze all imports' },
    { command: 'fidgetflo analyze imports src/ --external', description: 'Only npm packages' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
    const formatType = (ctx.flags.format as string) || 'text';
...
const depsCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze deps --outdated', description: 'Show outdated dependencies' },
    { command: 'fidgetflo analyze deps --security', description: 'Check for vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const boundariesCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze boundaries src/', description: 'Find code boundaries in src/' },
    { command: 'fidgetflo analyze boundaries -p 3 src/', description: 'Find 3 partitions' },
    { command: 'fidgetflo analyze boundaries -f dot -o graph.dot src/', description: 'Export to DOT format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const modulesCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze modules src/', description: 'Detect module communities' },
    { command: 'fidgetflo analyze modules -f dot -o modules.dot src/', description: 'Export colored DOT graph' },
    { command: 'fidgetflo analyze modules -m 3 src/', description: 'Only show communities with 3+ files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const dependenciesCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze dependencies src/', description: 'Build dependency graph' },
    { command: 'fidgetflo analyze dependencies -f dot -o deps.dot src/', description: 'Export to DOT' },
    { command: 'fidgetflo analyze dependencies -i .ts,.tsx src/', description: 'Only TypeScript files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const circularCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze circular src/', description: 'Find circular dependencies' },
    { command: 'fidgetflo analyze circular -s high src/', description: 'Only high severity cycles' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
export const analyzeCommand: Command = {
  ...
  examples: [
    { command: 'fidgetflo analyze ast src/', description: 'Analyze code with AST parsing' },
    { command: 'fidgetflo analyze complexity src/ --threshold 15', description: 'Find high-complexity files' },
    { command: 'fidgetflo analyze symbols src/ --type function', description: 'Extract all functions' },
    { command: 'fidgetflo analyze imports src/ --external', description: 'List npm dependencies' },
    { command: 'fidgetflo analyze diff --risk', description: 'Analyze diff with risk assessment' },
    { command: 'fidgetflo analyze boundaries src/', description: 'Find code boundaries using MinCut' },
    { command: 'fidgetflo analyze modules src/', description: 'Detect module communities with Louvain' },
    { command: 'fidgetflo analyze dependencies src/ --format dot', description: 'Export dependency graph as DOT' },
    { command: 'fidgetflo analyze circular src/', description: 'Find circular dependencies' },
    { command: 'fidgetflo analyze deps --security', description: 'Check dependency vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(`  ${output.dim('fidgetflo analyze ast src/')}                  # Full AST analysis`);
    output.writeln(`  ${output.dim('fidgetflo analyze ast src/index.ts -c')}       # Include complexity`);
    output.writeln(`  ${output.dim('fidgetflo analyze complexity src/ -t 15')}     # Flag high complexity`);
    output.writeln(`  ${output.dim('fidgetflo analyze symbols src/ --type fn')}    # Extract functions`);
    output.writeln(`  ${output.dim('fidgetflo analyze imports src/ --external')}   # Only npm imports`);
    output.writeln();

    output.writeln(output.bold('Graph Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('fidgetflo analyze boundaries src/')}            # Find natural code boundaries`);
    output.writeln(`  ${output.dim('fidgetflo analyze modules src/')}               # Detect module communities`);
    output.writeln(`  ${output.dim('fidgetflo analyze dependencies -f dot src/')}   # Export to DOT format`);
    output.writeln(`  ${output.dim('fidgetflo analyze circular src/')}              # Find circular deps`);
    output.writeln();

    output.writeln(output.bold('Diff Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('fidgetflo analyze diff --risk')}              # Risk assessment`);
    output.writeln(`  ${output.dim('fidgetflo analyze diff HEAD~1 --classify')}   # Classify changes`);
    output.writeln(`  ${output.dim('fidgetflo analyze diff main..feature')}       # Compare branches`);
    output.writeln();
