# 🌊 FidgetFlo v3.5: Enterprise AI Orchestration Platform

<div align="center">

![FidgetFlo Banner](fidgetflo/assets/fidgetflo-small.jpeg)
...
[![Monthly Downloads](https://img.shields.io/npm/dm/fidgetflo?style=for-the-badge&logo=npm&color=blue&label=Monthly%20Downloads)](https://www.npmjs.com/package/fidgetflo)
[![Total Downloads](https://img.shields.io/npm/dt/fidgetflo?style=for-the-badge&logo=npm&color=cyan&label=Total%20Downloads)](https://www.npmjs.com/package/fidgetflo)
...
> **Why FidgetFlo?** FidgetFlo is now FidgetFlo — named by Ruv, who loves Rust, flow states, and building things that feel inevitable. The "Ru" is the Ruv. The "flo" is the flow. Underneath, WASM kernels written in Rust power the policy engine, embeddings, and proof system. 6,000+ commits later, this is v3.5.

## Getting into the Flow

FidgetFlo is a comprehensive AI agent orchestration framework that transforms Claude Code into a powerful multi-agent development platform. It enables teams to deploy, coordinate, and optimize specialized AI agents working together on complex software engineering tasks.

### Self-Learning/Self-Optimizing Agent Architecture

```
User → FidgetFlo (CLI/MCP) → Router → Swarm → Agents → Memory → LLM Providers
                       ↑                          ↓
                       └──── Learning Loop ←──────┘
```
...
**RuVector Components** (included with FidgetFlo):
...
```bash
# Use RuVector via FidgetFlo
npx fidgetflo@latest hooks intelligence --status
```
...
⚡ **Plugs Into Claude Code** - Native integration via MCP (Model Context Protocol). Use fidgetflo commands directly in your Claude Code sessions with full tool access.
...
Complex swarms can drift from their original goals. FidgetFlo V3 includes anti-drift defaults that prevent agents from going off-task.
...
### Claude Code: With vs Without FidgetFlo

| Capability | Claude Code Alone | Claude Code + FidgetFlo |
|------------|-------------------|---------------------------|
...
npm install -g fidgetflo@latest --omit=optional
```

<details>
<summary>🤖 <strong>OpenAI Codex CLI Support</strong> — Full Codex integration with self-learning</summary>

FidgetFlo supports both **Claude Code** and **OpenAI Codex CLI** via the [@claude-flow/codex](https://www.npmjs.com/package/@claude-flow/codex) package, following the [Agentics Foundation](https://agentics.org) standard.

### Quick Start for Codex

```bash
# Initialize for Codex CLI (creates AGENTS.md instead of CLAUDE.md)
npx fidgetflo@latest init --codex

# Full Codex setup with all 137+ skills
npx fidgetflo@latest init --codex --full

# Initialize for both platforms (dual mode)
npx fidgetflo@latest init --dual
```
...
npx fidgetflo@latest mcp start

# Spawn a coding agent
npx fidgetflo@latest agent spawn -t coder --name my-coder

# Launch a hive-mind swarm with an objective
npx fidgetflo@latest hive-mind spawn "Implement user authentication"

# List available agent types
npx fidgetflo@latest agent list
```
...
npx fidgetflo@latest init upgrade

# Update AND add any missing skills/agents/commands
npx fidgetflo@latest init upgrade --add-missing
```
...
Add fidgetflo as an MCP server for seamless integration:
...
Once added, Claude Code can use all 313 fidgetflo MCP tools directly:
...
<details>
<summary>🆚 <strong>Why FidgetFlo v3?</strong></summary>

FidgetFlo v3 introduces **self-learning neural capabilities** that no other agent orchestration framework offers. While competitors require manual agent configuration and static routing, FidgetFlo learns from every task execution, prevents catastrophic forgetting of successful patterns, and intelligently routes work to specialized experts.

#### 🧠 Neural & Learning

| Feature | FidgetFlo v3 | CrewAI | LangGraph | AutoGen | Manus |
|---------|----------------|--------|-----------|---------|-------|
...
What makes FidgetFlo different from other agent frameworks? These 10 capabilities work together to create a system that learns from experience, runs efficiently on any hardware, and keeps working even when things go wrong.
...
Not every task needs the most powerful (and expensive) model. FidgetFlo analyzes each request and automatically routes it to the cheapest handler that can do the job well. Simple code transforms skip the LLM entirely using WebAssembly. Medium tasks use faster, cheaper models. Only complex architecture decisions use Opus.
...
FidgetFlo V3 integrates AgentDB v3 (3.0.0-alpha.10) providing 20+ memory controllers accessible via MCP tools and the CLI.
...
Connect FidgetFlo to your development environment.

<details>
<summary>🔌 <strong>MCP Setup</strong> — Connect FidgetFlo to Any AI Environment</summary>

FidgetFlo runs as an MCP (Model Context Protocol) server, allowing you to connect it to any MCP-compatible AI client. This means you can use FidgetFlo's 100+ agents, swarm coordination, and self-learning capabilities from Claude Desktop, VS Code, Cursor, Windsurf, ChatGPT, and more.

### Quick Add Command

```bash
# Start FidgetFlo MCP server in any environment
npx fidgetflo@latest mcp start
```
...
```json
{
  "mcpServers": {
    "fidgetflo": {
      "command": "npx",
      "args": ["fidgetflo@latest", "mcp", "start"],
      "env": {
...
For ChatGPT, you need a remote MCP server (not local stdio). Deploy fidgetflo to a server with HTTP transport:

```bash
# Start with HTTP transport
npx fidgetflo@latest mcp start --transport http --port 3000
```
...
2. Configure your fidgetflo MCP server
...
| `FIDGETFLO_LOG_LEVEL` | Logging level (debug, info, warn, error) | Optional |
| `FIDGETFLO_TOOL_GROUPS` | MCP tool groups to enable (comma-separated) | Optional |
| `FIDGETFLO_TOOL_MODE` | Preset tool mode (develop, pr-review, devops, etc.) | Optional |
...
export FIDGETFLO_TOOL_GROUPS=implement,test,fix,memory

# Or use a preset mode
export FIDGETFLO_TOOL_MODE=develop
```
...
**CLI Commands:**
```bash
npx fidgetflo hive-mind init                    # Initialize hive mind
npx fidgetflo hive-mind spawn "Build API"       # Spawn with objective
npx fidgetflo hive-mind spawn "..." --queen-type strategic --consensus byzantine
npx fidgetflo hive-mind status                  # Check status
npx fidgetflo hive-mind metrics                 # Performance metrics
npx fidgetflo hive-mind memory                  # Collective memory stats
npx fidgetflo hive-mind sessions                # List active sessions
```
...
**Enable Agent Teams:**
```bash
# Automatically enabled with fidgetflo init
npx fidgetflo@latest init
...
npx fidgetflo@latest hooks task-completed --task-id <id> --train-patterns
```
...
Install these optional plugins to extend FidgetFlo capabilities:
...
| **@claude-flow/plugin-gastown-bridge** | 0.1.0 | Gas Town orchestrator integration with WASM-accelerated formula parsing (instant (regex-based, no LLM call)), Beads sync, convoy management, and graph analysis. 20 MCP tools. | `npx fidgetflo@latest plugins install -n @claude-flow/plugin-gastown-bridge` |
| **@claude-flow/teammate-plugin** | 1.0.0-alpha.1 | Native TeammateTool integration for Claude Code v2.1.19+. BMSSP WASM acceleration, rate limiting, circuit breaker, semantic routing. 21 MCP tools. | `npx fidgetflo@latest plugins install -n @claude-flow/teammate-plugin` |
...
npx fidgetflo@latest plugins install -n @claude-flow/plugin-gastown-bridge
...
npx fidgetflo plugins list --installed
```
...
```bash
npx fidgetflo@latest worker dispatch --trigger audit --context "./src"
npx fidgetflo@latest worker status
```
...
| **Auto-Install** | `fidgetflo embeddings init` or `createEmbeddingServiceAsync()` | Zero config |
...
fidgetflo embeddings init

# Use larger model for higher quality
fidgetflo embeddings init --model all-mpnet-base-v2

# Semantic search
fidgetflo embeddings search -q "authentication patterns"
```
...
fidgetflo ruvector init --database mydb --user admin

# Check connection and schema status
fidgetflo ruvector status --verbose

# Run pending migrations
fidgetflo ruvector migrate --up

# Performance benchmark
fidgetflo ruvector benchmark --iterations 1000

# Optimize indices and vacuum
fidgetflo ruvector optimize --analyze

# Backup vector data
fidgetflo ruvector backup --output ./backup.sql
```
...
**Quick Commands:**
```bash
npx fidgetflo hive-mind init                                    # Initialize
npx fidgetflo hive-mind spawn "Build API" --queen-type tactical # Spawn swarm
npx fidgetflo hive-mind spawn "Research AI" --consensus byzantine --claude
npx fidgetflo hive-mind status                                  # Check status
```

**FidgetFlo Skill:** `/hive-mind-advanced` — Full hive mind orchestration
...
**Output Format:**
```
▊ FidgetFlo V3 ● ruvnet  │  ⎇ main  │  Opus 4.6  | ●42% ctx  | $0.15
🏗️ DDD [●●●●○] 4/5  ⚡ HNSW 150x  🤖 ◉ [12/8]  👥 3  🟢 CVE 3/3  💾 512MB  🧠 15%  📦 AgentDB ●1.2K vectors
```

| Indicator | Description | Source |
|-----------|-------------|--------|
| `▊ FidgetFlo V3` | Project header | Always shown |
...
| `[●●●●○]` | DDD domain progress bar | `.fidgetflo/metrics/v3-progress.json` |
...
| `🟢 CVE 3/3` | Security CVE remediation | `.fidgetflo/security/audit-status.json` |
...
Run `npx fidgetflo@latest init` — this generates `.claude/settings.json` with the correct statusline config and creates the helper script at `.claude/helpers/statusline.cjs`.
...
```bash
npx fidgetflo@latest init --update --settings
```
...
- `.fidgetflo/metrics/v3-progress.json` — DDD domain progress
- `.fidgetflo/metrics/swarm-activity.json` — Active agent counts
- `.fidgetflo/security/audit-status.json` — CVE remediation status
...
npx fidgetflo@latest daemon status

# Manually trigger a worker
npx fidgetflo@latest daemon trigger map

# Enable/disable workers
npx fidgetflo@latest daemon enable map audit optimize

# Stop daemon
npx fidgetflo@latest daemon stop
```
...
Complete command-line interface for all FidgetFlo operations.
...
# Run security scan
npx fidgetflo@latest security scan --depth full
...
Run `npx fidgetflo@latest doctor` to diagnose and fix common issues.
...
npx fidgetflo@latest doctor

# Run diagnostics with auto-fix
npx fidgetflo@latest doctor --fix

# Check specific component
npx fidgetflo@latest doctor --component memory

# Verbose output
npx fidgetflo@latest doctor --verbose
```

**Output Example:**

```
🩺 FidgetFlo Doctor v3.5

✅ Node.js      20.11.0 (required: 20+)
✅ npm          10.2.4 (required: 9+)
✅ Git          2.43.0
✅ Config       Valid fidgetflo.config.json
...
npx fidgetflo@latest embeddings init

# Generate embedding for text
npx fidgetflo@latest embeddings embed "authentication patterns"

# Batch embed multiple texts
npx fidgetflo@latest embeddings batch --file texts.txt

# Search with semantic similarity
npx fidgetflo@latest embeddings search "login flow" --top-k 5
```
...
|----------|----------------|--------------|
| **Code Review** | Get thorough reviews with security, performance, and style checks | `npx fidgetflo@latest agent spawn -t reviewer --name pr-review` |
| **Test Generation** | Auto-generate unit, integration, and e2e tests for existing code | `npx fidgetflo@latest agent spawn -t tester --name test-gen` |
| **Refactoring** | Safely restructure code while maintaining behavior | `npx fidgetflo@latest hive-mind spawn "Refactor user service to repository pattern"` |
| **Bug Fixing** | Diagnose and fix bugs with full context analysis | `npx fidgetflo@latest hive-mind spawn "Fix race condition in checkout flow"` |

### 🔒 Security & Compliance

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Security Audit** | Find vulnerabilities before attackers do | `npx fidgetflo@latest security scan --depth full` |
| **Dependency Scan** | Identify vulnerable packages and suggest upgrades | `npx fidgetflo@latest security cve --check` |
| **Compliance Check** | Ensure code meets security standards | `npx fidgetflo@latest security audit` |

### 🐝 Multi-Agent Swarms

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Feature Development** | Coordinate multiple agents on complex features | `npx fidgetflo@latest swarm init --topology hierarchical && npx fidgetflo@latest task orchestrate "Build user dashboard"` |
| **Large Refactors** | Parallel refactoring across many files without conflicts | `npx fidgetflo@latest swarm init --topology mesh --max-agents 8` |
| **Codebase Migration** | Migrate frameworks, languages, or patterns systematically | `npx fidgetflo@latest task orchestrate "Migrate from Express to Fastify" --strategy adaptive` |

### 📊 Performance & Optimization

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Performance Profiling** | Find and fix bottlenecks in your application | `npx fidgetflo@latest performance profile --target src/` |
| **Query Optimization** | Speed up slow database queries | `npx fidgetflo@latest performance benchmark --suite all` |
| **Memory Analysis** | Reduce memory usage and fix leaks | `npx fidgetflo@latest performance metrics` |

### 🔄 GitHub & DevOps

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **PR Management** | Review, approve, and merge PRs efficiently | `npx fidgetflo@latest hive-mind spawn "Review open PRs"` |
| **Issue Triage** | Categorize, prioritize, and assign issues automatically | `npx fidgetflo@latest hive-mind spawn "Triage new issues"` |
| **Release Management** | Coordinate releases with changelogs and versioning | `npx fidgetflo@latest hive-mind spawn "Prepare v2.0 release"` |
| **CI/CD Optimization** | Speed up pipelines and reduce flaky tests | `npx fidgetflo@latest hive-mind spawn "Optimize GitHub Actions workflow"` |

### 📋 Spec-Driven Development

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Generate Specs** | Create complete specifications before coding | `npx fidgetflo@latest hive-mind spawn "Create ADR for authentication system"` |
| **Validate Implementation** | Ensure code matches specifications | `npx fidgetflo@latest hooks progress --detailed` |
| **Track Compliance** | Monitor spec adherence across the team | `npx fidgetflo@latest progress sync` |

### 🧠 Learning & Intelligence

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Bootstrap Intelligence** | Train the system on your codebase patterns | `npx fidgetflo@latest hooks pretrain --depth deep` |
| **Optimize Routing** | Improve task-to-agent matching over time | `npx fidgetflo@latest hooks route "<task>" --include-explanation` |
| **Transfer Learning** | Apply patterns learned from other projects | `npx fidgetflo@latest hooks transfer <sourceProject>` |
...
FidgetFlo eliminates Claude Code's context window ceiling with a real-time memory management system that archives, optimizes, and restores conversation context automatically.
...
FidgetFlo intercepts the compaction lifecycle with three hooks that make context loss invisible:
...
|------|---------|---------|----------|
| 1 | **SQLite** (default) | `.fidgetflo/data/transcript-archive.db` | WAL mode, indexed queries, ACID, importance ranking |
...
| 4 | **JSON** (fallback) | `.fidgetflo/data/transcript-archive.json` | Zero dependencies, always works |

### Configuration

```bash
# Context Autopilot (all have sensible defaults)
FIDGETFLO_CONTEXT_AUTOPILOT=true        # Enable/disable autopilot (default: true)
FIDGETFLO_CONTEXT_WINDOW=200000         # Context window size in tokens
FIDGETFLO_AUTOPILOT_WARN=0.70           # Warning threshold (70%)
FIDGETFLO_AUTOPILOT_PRUNE=0.85          # Optimization threshold (85%)
FIDGETFLO_COMPACT_RESTORE_BUDGET=4000   # Max chars restored after compaction
FIDGETFLO_RETENTION_DAYS=30             # Auto-prune never-accessed entries
FIDGETFLO_AUTO_OPTIMIZE=true            # Importance ranking + pruning + sync
```
...
sqlite3 .fidgetflo/data/transcript-archive.db \
...
FidgetFlo uses RVF — a compact binary storage format that replaces the 18MB sql.js WASM dependency with pure TypeScript. No native compilation, no WASM downloads, works everywhere Node.js runs.
...
```bash
# Environment variables
FIDGETFLO_MEMORY_BACKEND=hybrid   # auto-selects RVF
FIDGETFLO_MEMORY_PATH=./data/memory

# Or via CLI
fidgetflo memory init --force
fidgetflo config set memory.backend hybrid
```
...
**Example Hook Output:**
```bash
$ npx fidgetflo@latest hooks pre-task --description "convert var to const in utils.ts"
...
npx fidgetflo@latest hooks pre-edit ./src/auth.ts
npx fidgetflo@latest hooks post-edit ./src/auth.ts --success true --train-patterns
```
...
npx fidgetflo@latest hooks route "refactor authentication to use JWT" --include-explanation

# Bootstrap intelligence from your codebase
npx fidgetflo@latest hooks pretrain --depth deep --model-type moe
```
...
npx fidgetflo@latest hooks session-start --session-id "feature-auth" --start-daemon

# End session and export learnings
npx fidgetflo@latest hooks session-end --export-metrics --persist-patterns
```
...
npx fidgetflo@latest hooks intelligence trajectory-start --task "implement OAuth2"

# Record successful action
npx fidgetflo@latest hooks intelligence trajectory-step --action "created token service" --quality 0.9

# End trajectory and trigger learning
npx fidgetflo@latest hooks intelligence trajectory-end --success true
...
npx fidgetflo@latest hooks worker dispatch --trigger audit --context "./src/auth"
...
npx fidgetflo@latest hooks model-route --task "fix typo in README"
# → Recommends: haiku (simple task, low complexity)

npx fidgetflo@latest hooks model-route --task "design distributed consensus system"
...
npx fidgetflo@latest hooks route "<task>" --include-explanation

# Start/end session with learning
npx fidgetflo@latest hooks session-start --start-daemon
npx fidgetflo@latest hooks session-end --persist-patterns

# View what the system has learned
npx fidgetflo@latest hooks metrics
npx fidgetflo@latest hooks intelligence stats
...
npx fidgetflo@latest hooks pretrain --depth deep
...
npx fidgetflo@latest memory export --format json --output ./patterns.json

# Export specific namespace
npx fidgetflo@latest memory export --namespace "security" --output ./security-patterns.json

# Export with embeddings (larger file, faster import)
npx fidgetflo@latest memory export --include-embeddings --output ./full-export.json

# Export agent configurations
npx fidgetflo@latest config export --scope project --output ./agent-configs.json

# Export session state
npx fidgetflo@latest session export --session-id "my-session" --output ./session.json
```
...
npx fidgetflo@latest memory import --input ./patterns.json

# Import and merge with existing (don't overwrite)
npx fidgetflo@latest memory import --input ./patterns.json --merge

# Import from another project
npx fidgetflo@latest hooks transfer --source-path ../other-project

# Import agent configurations
npx fidgetflo@latest config import --input ./agent-configs.json --scope project

# Restore session
npx fidgetflo@latest session restore --session-id "my-session"
```
...
npx fidgetflo@latest transfer-store search --query "authentication" --min-rating 4.0

# Download a pattern
npx fidgetflo@latest transfer-store download --id "auth-jwt-patterns-v2" --verify

# Publish your patterns
npx fidgetflo@latest transfer-store publish --input ./my-patterns.json --category "security"
```
...
# Filter by type
npx fidgetflo@latest plugins list --type integration

# Rate a plugin
npx fidgetflo@latest plugins rate --name @claude-flow/embeddings --rating 5

# Search for MCP tool plugins
npx fidgetflo@latest transfer plugin-search --type "mcp-tool" --verified

# Get plugin info
npx fidgetflo@latest transfer plugin-info --name "semantic-code-search"

# List official plugins
npx fidgetflo@latest transfer plugin-official
```
...
npx fidgetflo@latest transfer ipfs-resolve --name "/ipns/patterns.fidgetflo.io"

# Detect PII before publishing
npx fidgetflo@latest transfer detect-pii --content "$(cat ./patterns.json)"
```
...
curl -X POST "https://api.pinata.cloud/pinning/pinJSONToIPFS" \
  ...
  -d '{
    ...
    "pinataMetadata": {"name": "fidgetflo-learning-pattern"}
  }'
...
# Import all models
npx fidgetflo@latest transfer import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Import specific category
npx fidgetflo@latest neural import --model security-review-patterns --source ipfs

# Use patterns in routing
npx fidgetflo@latest hooks route --task "review authentication code" --use-patterns
```
...
npx fidgetflo@latest transfer-store download --id "security-essentials" --apply
```
...
npx fidgetflo@latest neural list

# List models by category
npx fidgetflo@latest neural list --category security

# Train with WASM acceleration
npx fidgetflo@latest neural train -p coordination -e 100 --wasm --flash --contrastive

# Train security patterns
npx fidgetflo@latest neural train -p security --wasm --contrastive

# Benchmark WASM performance
npx fidgetflo@latest neural benchmark -d 256 -i 1000

# Import pre-trained models
npx fidgetflo@latest neural import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Export trained patterns to IPFS
npx fidgetflo@latest neural export --ipfs --sign
```
...
# Via CLI
npx fidgetflo@latest skill run github-code-review
npx fidgetflo@latest skill list
npx fidgetflo@latest skill info sparc-methodology
```
...
|---------|--------------|---------|
| `issues list` | See all issues and their status | `npx fidgetflo@latest issues list` |
| `issues claim` | Claim an issue for yourself/agent | `npx fidgetflo@latest issues claim #123 --as coder-1` |
| `issues release` | Release your claim | `npx fidgetflo@latest issues release #123` |
| `issues handoff` | Hand off to another worker | `npx fidgetflo@latest issues handoff #123 --to reviewer` |
| `issues status` | Update progress on claimed work | `npx fidgetflo@latest issues status #123 --progress 75` |
| `issues stealable` | List abandoned/stuck issues | `npx fidgetflo@latest issues stealable` |
| `issues steal` | Take over stealable issue | `npx fidgetflo@latest issues steal #123` |
| `issues load` | View agent workloads | `npx fidgetflo@latest issues load` |
| `issues rebalance` | Redistribute work evenly | `npx fidgetflo@latest issues rebalance --dry-run` |
| `issues board` | Visual board view | `npx fidgetflo@latest issues board` |

### Visual Board View

```bash
npx fidgetflo@latest issues board
```
...
npx fidgetflo@latest issues handoff #123 \
  --to security-architect \
  --reason "Needs security review" \
  --progress 80

# 2. Target accepts handoff
npx fidgetflo@latest issues accept #123 --as security-architect
...
```bash
# View current load
npx fidgetflo@latest issues load
...
# Auto-rebalance
npx fidgetflo@latest issues rebalance
```
...
| Command | What It Does | Example |
|---------|--------------|---------|
| `route task` | Get agent recommendation | `npx fidgetflo@latest route task "implement OAuth2"` |
| `route explain` | Understand routing decision | `npx fidgetflo@latest route explain "task"` |
| `route coverage` | Route based on test coverage | `npx fidgetflo@latest route coverage` |

### Example: Route a Task

```bash
npx fidgetflo@latest route task "refactor authentication to use JWT"
...
```bash
npx fidgetflo@latest route coverage
...
npx fidgetflo@latest hooks route "implement caching layer" --include-explanation

# Record outcome for learning
npx fidgetflo@latest hooks post-task --task-id "task-123" --success true --agent coder
```
...
Use FidgetFlo packages directly in your applications.

<details>
<summary>💻 <strong>Programmatic SDK</strong> — Use FidgetFlo in Your Code</summary>

Use FidgetFlo packages directly in your TypeScript/JavaScript applications.
...
npx fidgetflo@latest memory init --force

# Store patterns
npx fidgetflo@latest memory store --key "pattern-auth" --value "JWT authentication with refresh tokens"
npx fidgetflo@latest memory store --key "pattern-cache" --value "Redis caching for API responses"

# Build HNSW index for HNSW-indexed search
npx fidgetflo@latest memory search --query "authentication" --build-hnsw

# Semantic search (uses HNSW if built)
npx fidgetflo@latest memory search --query "how to cache data" --limit 5

# List and manage entries
npx fidgetflo@latest memory list --namespace patterns
npx fidgetflo@latest memory stats
```
...
```bash
# Generate embedding
fidgetflo embeddings embed "Your text here"

# Batch embed from file
fidgetflo embeddings batch documents.txt -o embeddings.json

# Similarity search
fidgetflo embeddings search "query" --index ./vectors

# Document chunking
fidgetflo embeddings chunk document.txt --strategy sentence --max-size 512

# Normalize embeddings
fidgetflo embeddings normalize embeddings.json --type l2 -o normalized.json

# Convert to hyperbolic
fidgetflo embeddings hyperbolic embeddings.json -o poincare.json

# Neural operations
fidgetflo embeddings neural drift --baseline "context" --input "check"
fidgetflo embeddings neural store --id mem-1 --content "data"
fidgetflo embeddings neural recall "query" --top-k 5

# Model management
fidgetflo embeddings models list
fidgetflo embeddings models download all-MiniLM-L6-v2

# Cache management
fidgetflo embeddings cache stats
fidgetflo embeddings cache clear --older-than 7d
```
...
Core infrastructure packages powering FidgetFlo's intelligence layer.
...
FidgetFlo v3 is built on top of **[agentic-flow](https://github.com/ruvnet/agentic-flow)**, a production-ready AI agent orchestration platform. This deep integration provides instant (regex-based, no LLM call) code transformations, learning memory, and geometric intelligence.
...
The agentic-flow ecosystem exposes MCP tools across packages (fidgetflo CLI provides 314 tools):
...
FidgetFlo automatically leverages agentic-flow for:
...
```typescript
// FidgetFlo automatically uses agentic-flow optimizations
import { getTokenOptimizer } from '@claude-flow/integration';
...
FidgetFlo includes a dedicated `/agentic-jujutsu` skill for AI-powered version control:
...
**RuVector** is a high-performance distributed vector database combining vector search, graph queries, and self-learning neural networks. Written in Rust with Node.js/WASM bindings, it powers FidgetFlo's intelligence layer with native speed.
...
npx fidgetflo ruvector setup --output ./my-ruvector
...
docker run -d \
  ...
  -e POSTGRES_PASSWORD=fidgetflo-test \
...
FidgetFlo automatically uses RuVector when available:

```typescript
// FidgetFlo detects and uses native ruvector
...
npx fidgetflo ruvector setup --output ./mydir   # Custom directory
npx fidgetflo ruvector setup --print            # Preview files

# Import from sql.js/JSON to PostgreSQL
npx fidgetflo ruvector import --input data.json              # Direct import
npx fidgetflo ruvector import --input data.json --output sql # Dry-run (generate SQL)

# Other RuVector commands
npx fidgetflo ruvector status --verbose         # Check connection
npx fidgetflo ruvector benchmark --vectors 10000 # Performance test
npx fidgetflo ruvector optimize --analyze       # Optimization suggestions
npx fidgetflo ruvector backup --output backup.sql # Backup data
...
Flow Nexus is a **cloud platform** for deploying and scaling FidgetFlo beyond your local machine.

### What Flow Nexus Provides

| Feature | Local FidgetFlo | + Flow Nexus |
|---------|-------------------|--------------|
...
# Or via CLI
npx fidgetflo@latest nexus swarm deploy \
  --topology hierarchical \
  --max-agents 50 \
  --region us-east-1
```
...
```bash
# Create sandbox
npx fidgetflo@latest nexus sandbox create --language python

# Execute code safely
npx fidgetflo@latest nexus sandbox exec --code "print('Hello')"

# Cleanup
npx fidgetflo@latest nexus sandbox destroy
```
...
# 2. Get API key
# 3. Configure
npx fidgetflo@latest nexus configure --api-key <key>
...
# Define pipeline
npx fidgetflo@latest stream-chain create \
...
```bash
# Run the pipeline
npx fidgetflo@latest stream-chain run feature-pipeline \
  --input '{"requirements": "Add user dashboard with analytics"}'

# Monitor progress
npx fidgetflo@latest stream-chain status feature-pipeline
```
...
# Via CLI
npx fidgetflo@latest pair start --mode navigator
```
...
npx fidgetflo@latest pair switch

# Get AI explanation
npx fidgetflo@latest pair explain

# Run tests
npx fidgetflo@latest pair test

# End session with summary
npx fidgetflo@latest pair end
```
...
```bash
# Basic threat scan
npx fidgetflo@latest security defend -i "ignore previous instructions"

# Scan a file
npx fidgetflo@latest security defend -f ./user-prompts.txt

# Quick scan (faster)
npx fidgetflo@latest security defend -i "some text" --quick

# JSON output
npx fidgetflo@latest security defend -i "test" -o json

# View statistics
npx fidgetflo@latest security defend --stats
...
AI-optimized browser automation integrating [agent-browser](https://github.com/AugmentCode/agent-browser) with fidgetflo for intelligent web automation, trajectory learning, and multi-agent browser coordination.
...
Automated release management, versioning, and CI/CD for FidgetFlo packages.
...
# Performance report
npx fidgetflo@latest performance report

# Benchmark specific suite
npx fidgetflo@latest performance benchmark --suite memory
```
...
$env:FIDGETFLO_MODE = "integration"
```
...
export FIDGETFLO_SECURITY_MODE="strict"
```
...
export FIDGETFLO_MEMORY_PATH="./data"
```
...
|----------|-------------|---------|
| `FIDGETFLO_MODE` | Operation mode (`development`, `production`, `integration`) | `development` |
| `FIDGETFLO_ENV` | Environment name for test/dev isolation | - |
| `FIDGETFLO_DATA_DIR` | Root data directory | `./data` |
| `FIDGETFLO_MEMORY_PATH` | Directory for persistent memory storage | `./data` |
| `FIDGETFLO_MEMORY_TYPE` | Memory backend type (`json`, `sqlite`, `agentdb`, `hybrid`) | `hybrid` |
| `FIDGETFLO_SECURITY_MODE` | Security level (`strict`, `standard`, `permissive`) | `standard` |
| `FIDGETFLO_LOG_LEVEL` | Logging verbosity (`debug`, `info`, `warn`, `error`) | `info` |
| `FIDGETFLO_CONFIG` | Path to configuration file | `./fidgetflo.config.json` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `FIDGETFLO_MAX_AGENTS` | Default concurrent agent limit | `15` |
| `FIDGETFLO_TOPOLOGY` | Default swarm topology (`hierarchical`, `mesh`, `ring`, `star`) | `hierarchical` |
| `FIDGETFLO_HEADLESS` | Run in headless mode (no interactive prompts) | `false` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `FIDGETFLO_MCP_PORT` | MCP server port | `3000` |
| `FIDGETFLO_MCP_HOST` | MCP server host | `localhost` |
| `FIDGETFLO_MCP_TRANSPORT` | Transport type (`stdio`, `http`, `websocket`) | `stdio` |

### Vector Search (HNSW)

| Variable | Description | Default |
|----------|-------------|---------|
| `FIDGETFLO_HNSW_M` | HNSW index M parameter (connectivity, higher = more accurate) | `16` |
| `FIDGETFLO_HNSW_EF` | HNSW search ef parameter (accuracy, higher = slower) | `200` |
| `FIDGETFLO_EMBEDDING_DIM` | Vector embedding dimensions | `384` |
...
| `GCS_PREFIX` | Prefix for stored files | `fidgetflo-patterns` |

### Auto-Update System

| Variable | Description | Default |
|----------|-------------|---------|
| `FIDGETFLO_AUTO_UPDATE` | Enable/disable auto-updates | `true` |
| `FIDGETFLO_FORCE_UPDATE` | Force update check | `false` |
...
| `FIDGETFLO_TOKEN` | Internal authentication token | Optional |
...
```bash
# Core
FIDGETFLO_MODE=development
FIDGETFLO_LOG_LEVEL=info
FIDGETFLO_MAX_AGENTS=15
...
# Memory
FIDGETFLO_MEMORY_TYPE=hybrid
FIDGETFLO_MEMORY_PATH=./data

# Vector Search
FIDGETFLO_HNSW_M=16
FIDGETFLO_HNSW_EF=200
...
FidgetFlo looks for configuration in this order:
1. `./fidgetflo.config.json` (project root)
2. `~/.config/fidgetflo/config.json` (user config)
...
```json
{
  ...
  "logging": {
    ...
    "format": "json",
    "destination": "file",
    "filePath": "/var/log/fidgetflo/production.log"
  },
...
npx fidgetflo@latest config list

# Get specific value
npx fidgetflo@latest config get --key memory.type

# Set configuration value
npx fidgetflo@latest config set --key swarm.maxAgents --value 10

# Export configuration
npx fidgetflo@latest config export > my-config.json

# Import configuration
npx fidgetflo@latest config import --file my-config.json

# Reset to defaults
npx fidgetflo@latest config reset --key swarm
...
npx fidgetflo@latest hooks pretrain
```

**Windows path issues**
```powershell
# Use forward slashes or escape backslashes
$env:FIDGETFLO_MEMORY_PATH = "./data"
# Or use absolute path
$env:FIDGETFLO_MEMORY_PATH = "C:/Users/name/fidgetflo/data"
```
...
node --expose-gc node_modules/.bin/fidgetflo
# Reduce HNSW parameters for lower memory
export FIDGETFLO_HNSW_M=8
export FIDGETFLO_HNSW_EF=100
```
...
| Change | V2 | V3 | Impact |
|--------|----|----|--------|
| **Package Structure** | `fidgetflo` | `@claude-flow/*` (scoped) | Update imports |
...
| **Config Format** | `.fidgetflo/config.json` | `fidgetflo.config.json` | Update path |
...
cp -r ./.fidgetflo ./.fidgetflo-backup-v2

# STEP 2: Check migration status
npx fidgetflo@latest migrate status

# STEP 3: Run migration with dry-run first
npx fidgetflo@latest migrate run --dry-run

# STEP 4: Execute migration
npx fidgetflo@latest migrate run --from v2

# STEP 5: Verify migration
npx fidgetflo@latest migrate verify
...
|------------|------------|-------|
| `fidgetflo start` | `fidgetflo mcp start` | MCP is explicit |
| `fidgetflo init` | `fidgetflo init --wizard` | Interactive mode |
| `fidgetflo spawn <type>` | `fidgetflo agent spawn -t <type>` | Nested under `agent` |
| `fidgetflo swarm create` | `fidgetflo swarm init --topology mesh` | Explicit topology |
...
**V2 Config (`.fidgetflo/config.json`)**:
```json
{
  "mode": "basic",
...
**V3 Config (`fidgetflo.config.json`)**:
```json
{
  "version": "3.0.0",
  "memory": {
...
```typescript
// V2 (deprecated)
import { ClaudeFlow, Agent, Memory } from 'fidgetflo';
...
npx fidgetflo@latest migrate rollback --list

# Rollback to V2
npx fidgetflo@latest migrate rollback --to v2
...
- [ ] Verify all agents spawn correctly: `npx fidgetflo@latest agent list`
- [ ] Check memory search works: `npx fidgetflo@latest memory search -q "test"`
- [ ] Confirm MCP server starts: `npx fidgetflo@latest mcp start`
- [ ] Run doctor diagnostics: `npx fidgetflo@latest doctor`
- [ ] Test a simple swarm: `npx fidgetflo@latest swarm init --topology mesh`
- [ ] Bootstrap learning: `npx fidgetflo@latest hooks pretrain`
...
| `Config not found` | Path change | Rename to `fidgetflo.config.json` |
