...
describe('EnforcementGates', () => {
  ...
  describe('evaluateSecrets', () => {
    it('should detect API keys', () => {
      const result = gates.evaluateSecrets('const apiKey = "GENERIC_KEY_REDACTED-AUDIT-2026-04-25"');
      expect(result).not.toBeNull();
      expect(result!.decision).toBe('block');
    ...
    it('should detect passwords', () => {
      const result = gates.evaluateSecrets('password = "GENERIC_KEY_REDACTED-AUDIT-2026-04-25"');
      expect(result).not.toBeNull();
      expect(result!.decision).toBe('block');
    });
