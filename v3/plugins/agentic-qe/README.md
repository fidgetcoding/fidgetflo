...
This plugin adds 58 AI agents to FidgetFlo that handle all aspects of software quality:
...
**Via FidgetFlo CLI (recommended):**
```bash
npx fidgetflo plugins install --name @claude-flow/plugin-agentic-qe
```
...
```bash
npx fidgetflo@v3alpha mcp call aqe/generate-tests \
...
```bash
npx fidgetflo@v3alpha mcp call aqe/tdd-cycle \
...
```bash
npx fidgetflo@v3alpha mcp call aqe/security-scan \
  --targetPath ./src \
  --scanType sast \
...
```bash
npx fidgetflo@v3alpha mcp call aqe/predict-defects \
...
npx fidgetflo@v3alpha mcp call aqe/chaos-inject \
