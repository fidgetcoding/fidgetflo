...
```bash
npx fidgetflo hive-mind init --topology hierarchical-mesh --consensus raft
```

### Spawn Queen
```bash
npx fidgetflo hive-mind spawn --role queen --name coordinator
```

### Check Consensus Status
```bash
npx fidgetflo hive-mind consensus --status
```

### View Sessions
```bash
npx fidgetflo hive-mind sessions --active
```
