...
### Train Patterns
```bash
npx fidgetflo neural train --model-type moe --epochs 10
```
...
### View Patterns
```bash
npx fidgetflo neural patterns --type all
```

### Predict
```bash
npx fidgetflo neural predict --input "task description"
```

### Optimize
```bash
npx fidgetflo neural optimize --target latency
```
