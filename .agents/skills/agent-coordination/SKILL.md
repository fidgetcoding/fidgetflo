...
### List Agents
```bash
npx fidgetflo agent list --filter active
```

### Agent Status
```bash
npx fidgetflo agent status --id agent-123
```

### Agent Metrics
```bash
npx fidgetflo agent metrics --id agent-123
```

### Stop Agent
```bash
npx fidgetflo agent stop --id agent-123
```

### Pool Management
```bash
npx fidgetflo agent pool --size 5 --type coder
```
