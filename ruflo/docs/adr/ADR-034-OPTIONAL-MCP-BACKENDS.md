...
ADR-033 added ruvector (61 tools) and fidgetflo (215 tools) as default MCP backends. Users also want access to additional AI agent capabilities:
...
Add three optional MCP backends that can be enabled via environment variables. Unlike ruvector/fidgetflo (enabled by default), these are **disabled by default** and require explicit API keys.
...
| fidgetflo | `ENABLE_RUFLO` | None | `npx fidgetflo mcp start` | **enabled** |
...
ruflo__agent_spawn         → fidgetflo MCP
...
- [ADR-033: RuVector + FidgetFlo MCP Integration](ADR-033-RUVECTOR-FIDGETFLO-MCP-INTEGRATION.md)
