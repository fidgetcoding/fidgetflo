...
Tools prefixed with backend name (e.g., \`fidgetflo__agent_spawn\`). Always use the full prefixed name.
...
- **guidance** — Get help on tool groups or specific tool usage.
  ...
  \`{"topic": "overview"}\` or \`{"topic": "tool", "tool_name": "fidgetflo__agent_spawn"}\`
...
7. **Code task** → \`ruvector__hooks_route\` + \`fidgetflo__agent_spawn\`
8. **Remember something** → \`fidgetflo__memory_store\` / \`fidgetflo__memory_search\`
...
async function handleAutopilot(req, res, provider, body) {
  ...
  const priorityPrefixes = ['ruvector__hooks', 'ruvector__memory', 'fidgetflo__memory', 'fidgetflo__agent'];
